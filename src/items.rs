...
pub struct ScreenItems {
    str_vec: Vec<String>,  // display strings
...
impl ScreenItems {
    pub fn new(str_vec: &Vec<String>, rows: i32) -> ScreenItems {
        let mut fscreen = ScreenItems {
            str_vec: str_vec.clone(),
            rows,
            max_display_items: 0,
            num_items: 0,
            num_display_items: 0,
    ...
    fn init(&mut self) {
        ...
        self.num_items = self.str_vec.len() as i32;
...
impl ScreenItems {
    ...
    pub fn str_vec_display(&self) -> &[String] {
        let start = self.start as usize;
        let stop = self.end as usize + 1;
        &self.str_vec[start..stop]
    }
}
